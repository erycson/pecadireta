...
use App\Models\FornecedorTipo as Model;

class FornecedoresTipos extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('fornecedores', fn ($fornecedorTipo) => $fornecedorTipo->fornecedores_count)
            ->editColumn('criado_em', fn ($fornecedorTipo) => $fornecedorTipo->criado_em->format('d/m/Y'))
            ->addColumn('acoes', fn ($fornecedorTipo) => sprintf('
                    <div class="row gx-1 justify-content-end flex-nowrap">
                        <div class="col-auto">
                            <a href="%s" class="btn btn-sm btn-secondary rounded-pill px-3">Editar</a>
                        </div>
                    </div>
                ', route('painel.fornecedores-tipos.edit', [$fornecedorTipo]))
            )
            ->rawColumns(['acoes']);
    }
